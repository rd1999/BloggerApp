...
const Preview = ({route, navigation}) => {
    ...
    const {title, titleImage, html, name, type, hashtags} = route.params
    ...
    return(
        <Layout style={{flex: 1}}>
            ...
            <ScrollView style={styles.containerStyle} showsVerticalScrollIndicator={false}>
                <Text category='h1' style={{marginLeft: 10}}>{title}</Text>
                <ImageBackground 
                ...
                {type==='preview'?<Text style={{fontSize: 13}}>{date}-{month}-{year}</Text>:null}
